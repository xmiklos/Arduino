...
Value<int>            voltage_limit( 32, 19, 33 );
...
Value<int>            blink_speed( 500, 0, 2000 );
...
void loop() {
  ...
  if (line_active.get()) {
    ...
    } else if (line_selected.get() == 1) {
      current_limit.set( current_limit.get() + direction() );
    } else if (line_selected.get() == 3) {
      blink_speed.set( blink_speed.get() + direction() * 10 );
    }
  } else {
