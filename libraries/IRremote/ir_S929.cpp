/*
Assuming the protocol we are adding is for the (imaginary) manufacturer:  S929
...
1. Copy this file to : ir_S929.cpp

2. Replace all occurrences of "S929" with the name of your protocol.
...
1. Open IRremote.h and make the following changes:
   REMEMEBER to change occurences of "S929" with the name of your protocol

   A. At the top, in the section "Supported Protocols", add:
      #define DECODE_S929  1
      #define SEND_S929    1
   ...
   C. Further down in "Main class for receiving IR", add:
      //......................................................................
      #if DECODE_S929
          bool  decodeS929 (decode_results *results) ;
      #endif

   D. Further down in "Main class for sending IR", add:
      //......................................................................
      #if SEND_S929
          void  sendS929 (unsigned long data,  int nbits) ;
      #endif
...
2. Now open irRecv.cpp and make the following change:
   ...
   A. In the function IRrecv::decode(), add:
      #ifdef DECODE_NEC
          DBG_PRINTLN("Attempting S929 decode");
          if (decodeS929(results))  return true ;
      #endif
...
3. Open MyDocuments\Arduino\libraries\IRremote\examples\IRrecvDumpV2.ino
   ...
   A. In the encoding() function, add:
      case S929:    Serial.print("S929");     break ;
...
#define HDR_MARK    1254  // The length of the Header:Mark
#define HDR_SPACE   775  // The lenght of the Header:Space

#define BIT_MARK    225  // The length of a Bit:Mark
#define ONE_SPACE   925  // The length of a Bit:Space for 1's
#define ZERO_SPACE  425  // The length of a Bit:Space for 0's
...
//
#if SEND_S929
void  IRsend::sendS929 (unsigned long data,  int nbits)
{
...
//
#if DECODE_S929
bool  IRrecv::decodeS929 (decode_results *results)
{
	...
	if (irparams.rawlen != 34)  { return false; }
